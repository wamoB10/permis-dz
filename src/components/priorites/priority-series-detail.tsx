...
export default function PrioritySeriesDetailClient({ series }: { series: PrioritySeries }) {
  const router = useRouter()
  const currentId = series.id
  const prevSeries = prioritySeriesList.find(s => s.id === currentId - 1 && s.items.length > 0)
  const nextSeries = prioritySeriesList.find(s => s.id === currentId + 1 && s.items.length > 0)
  ...
  return (
    <div className="container py-10">
      ...
      <p className="text-muted-foreground mb-8 text-center">8 situations à connaître</p>
